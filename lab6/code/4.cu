...
__global__ void im2col(unsigned int n, float *matAc, float *matA, int radiusF, int countF, int L, int M, int K, int C,int H)
{
...
__global__ void GEMM_cuda(double *a,double *b, double *c, int m, int n, int k)
{ 
...
int main(int argc, char const *argv[])
{
    ...
    im2col<<<gridSize, blockSize>>>(KERNELS_NUM, devAc, devA, radiusF, countF, L, M, K, C,H);    
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&time_gpu, start, stop);
    cudaMemcpy(retAc, devAc, sizeAc, cudaMemcpyDeviceToHost);
    printf("im2col CONVOLUTION TIME:        %f ms\n", time_gpu);
